...
class YAMLConfig(BaseConfig):
    ...
    #         if self.yaml_cfg['evaluator']['type'] == 'CocoEvaluator' or self.yaml_cfg['evaluator']['type'] == 'GroundingEvaluator':
    ...
    # def evaluator(self):
    #     if self._evaluator is None and 'evaluator' in self.yaml_cfg:
    #         evaluator_type = self.yaml_cfg['evaluator'].get('type', 'CocoEvaluator')
            
    #         # ✅ 两种评测器用完全相同的方式初始化
    #         if evaluator_type in ['CocoEvaluator', 'GroundingEvaluator']:
    #             from ..data import get_coco_api_from_dataset
    #             base_ds = get_coco_api_from_dataset(self.val_dataloader.dataset)
    #             if isinstance(base_ds, tuple):
    #                 coco_gts, _ = base_ds
    #                 self._evaluator = [
    #                     create('evaluator', self.global_cfg, coco_gt=coco_gt)
    #                     for coco_gt in coco_gts
    #                 ]
    #             else:
    #                 self._evaluator = create('evaluator', self.global_cfg, coco_gt=base_ds)
    ...
    #                 f"Evaluator type '{evaluator_type}' not supported. "
    #                 f"Available: ['CocoEvaluator', 'GroundingEvaluator']"
    #             )
    @property
    def evaluator(self):
        if self._evaluator is None and 'evaluator' in self.yaml_cfg:
            evaluator_cfg = self.yaml_cfg['evaluator']
            from ..data import get_coco_api_from_dataset
            base_ds = get_coco_api_from_dataset(self.val_dataloader.dataset)

            # 支持为 ConcatDataset 提供按顺序的 evaluator 列表
            if isinstance(evaluator_cfg, list):
                coco_gts = base_ds[0] if isinstance(base_ds, tuple) else [base_ds]
                assert len(evaluator_cfg) == len(coco_gts), \
                    f"Number of evaluators ({len(evaluator_cfg)}) must match number of datasets ({len(coco_gts)})"

                evaluators = []
                for cfg, coco_gt in zip(evaluator_cfg, coco_gts):
                    cfg_local = copy.deepcopy(cfg)
                    cfg_type = cfg_local.pop('type', 'CocoEvaluator')

                    if cfg_type not in self.global_cfg:
                        raise NotImplementedError(
                            f"Evaluator type '{cfg_type}' not supported. "
                            f"Available: ['CocoEvaluator', 'GroundingEvaluator']"
                        )

                    # 只对该 evaluator 条目做浅拷贝并覆写参数，避免 deepcopy 模块对象
                    local_cfg = self.global_cfg.copy()
                    local_cfg[cfg_type] = dict(local_cfg[cfg_type])
                    local_cfg[cfg_type].update(cfg_local)
                    # 需要显式写入 coco_gt，因为 create(name, ...) 不会自动合并 kwargs
                    local_cfg[cfg_type]['coco_gt'] = coco_gt

                    evaluators.append(create(cfg_type, local_cfg, coco_gt=coco_gt))

                self._evaluator = evaluators
                
            else:
                evaluator_type = evaluator_cfg.get('type', 'CocoEvaluator')
                supported_evaluators = ['CocoEvaluator', 'GroundingEvaluator']
                if evaluator_type in supported_evaluators:
                    if isinstance(base_ds, tuple):
                        coco_gts, _ = base_ds
                        self._evaluator = [
                            create('evaluator', self.global_cfg, coco_gt=coco_gt)
                    ...
                    raise NotImplementedError(
                        f"Evaluator type '{evaluator_type}' not supported. "
                        f"Available: {supported_evaluators}"
                    )
