...
__all__ = ['CocoDetection_W_Caption']


@register()
class CocoDetection_W_Caption(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False,
        ...
        super(CocoDetection_W_Caption, self).__init__(img_folder, ann_file)
    ...
    def load_item(self, idx):
        image, target = super(CocoDetection_W_Caption, self).__getitem__(idx)
        ...
        ann_list = super(CocoDetection_W_Caption, self).__getitem__(idx)[1]
        if isinstance(ann_list, list) and len(ann_list) > 0:
            original_ann = ann_list[0]
            target["captions"] = [original_ann.get("caption", None)]
            
            if self.attributes_align_use:
                attr_tmp_list = []
                attributes = original_ann.get("attributes", None)
                for attr in attributes:
                    attr_tmp_list.append(attr['description'])
                target["caption_attributes"] = attr_tmp_list
        else:
            raise ValueError('dataset error, please check your data.')
...
if __name__ == "__main__":
    dataset = CocoDetection_W_Caption(
        img_folder="../datasets/LLM_Caption_Parse/images/AerialVG/",
        ann_file="../datasets/LLM_Caption_Parse/annotations_grounding/Aerial_VG_COCO/vg_train_odvg_coco.json",
        transforms=None,
        return_masks=False
    )
    print(len(dataset))
    img, target = dataset[0]
    print(target["G_caption"])
    print(target["boxes"])
